...
func StopDesiredAgent(da *model.DesiredAgent) {
	if !file.IsExist(da.ControlFilepath) {
		log.Println(da.ControlFilepath)
		log.Println("can not find control file path")

	}
	ControlStopIn(da.AgentVersionDir)
}

func StopAgentOf(agentDir, newVersion string) error {
...
func ControlStopIn(workdir string) error {
	if !file.IsExist(workdir) {
		log.Println("workdir is not exit")
		return nil
	}
	...
	log.Println("try to stop previous version  failed")
	return err
}
